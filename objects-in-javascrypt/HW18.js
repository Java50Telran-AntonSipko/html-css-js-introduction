...
function getMostPopulatedCountry(employees) {
    const occur=getOccurrences(employees);
     const arrayOccurrences = Object.entries(occur);
     arrayOccurrences.sort(bypopulation);
     return arrayOccurrences[0][0];
    //returns country with most amount of employees
}
function getOccurrences(employees){
    const res={};
    employees.forEach((employee,index)=>{
        let country=employees[index].address.country;
        if(!res[country]){
            res[country]=1;
        }else{
            res[country]++
...
function bypopulation(entry1,entry2){
    let res = entry2[1] - entry1[1];
return res;
}
function getMostPupulatedCountries(employees, counter) {
    const occur=getOccurrences(employees);
     ...
     for(let i=0;i<counter;i++){
        popularCountries[i]=arrayOccurrences[i][0];
     }
